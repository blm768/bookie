...
===Included sender plugins
- <tt>standalone</tt>: processes data from standalone Linux machines using acct(5)-formatted files (i.e. pacct)
- <tt>torque_cluster</tt>: processes data from TORQUE logs ($TORQUEROOT/server_priv/accounting/YYYYMMDD)
...
===Included formatter plugins
- <tt>stdout</tt>: the default formatter; prints data on standard output
- <tt>comma_dump</tt>: outputs data in CSV format
- <tt>spreadsheet</tt>: outputs data in Microsoft Excel (*.xls) format
